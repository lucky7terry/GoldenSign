...
| `WORD_SOURCE_FPS` | (비움) | 되돌릴 프레임레이트. 비우면 구간의 관측 간격을 쓴다 |
...
12.7fps 로 도는데(#44) 스트림은 30fps 라 처리하지 못한 프레임은 버려지고,
얼마나 버려지는지가 순간마다 다르다. 그대로 넣으면 궤적이 시간축으로
일그러진다 — 촘촘하게 살아남은 구간은 느리게, 성기게 살아남은 구간은
빠르게 움직인 것처럼 보인다.
...
그래서 간격이 이미 균일하면 항등이 되고, 불규칙할 때만 편다. 도착하지 않은
프레임을 지어내지 않는다.

영상 5개(WORD0001, 5시점)를 10fps 로 떨어뜨려 실측했다.

| 드롭 방식 | `[1]` 없음 | `[1]` 관측 간격 | `[1]` 30fps 고정 |
|---|---|---|---|
| uniform (균일) | 5/5 · 0.714 | 5/5 · **0.714** | 5/5 · 0.708 |
| random (불규칙) | **4/5** · 0.637 | 5/5 · **0.782** | 5/5 · 0.776 |
| stall (0.5초 멈춤) | 5/5 · 0.752 | 5/5 · **0.778** | 5/5 · 0.759 |

불규칙하게 버려질 때 `[1]` 이 없으면 확신도가 무너지고 한 번은 틀렸다
(R 시점, "허리" 0.298, 2위와의 격차 0.019). 균일할 때는 세 방법이 같다.

`WORD_SOURCE_FPS` 에 값을 주면 그 프레임레이트로 되돌린다. 30 을 주면 속도
특징이 학습과 같은 "1/30초당 변위"가 되지만, 실측에서는 그것이 이득이
아니었다 — 없던 프레임을 보간으로 지어내는 대가가 더 컸다.
