...
스케일과 결측 보간은 넘겨받은 구간 전체를 보고 계산한다:
    학습은 영상 한 편을 통째로 보고 계산했다. 여기서는 호출자가 준 구간이
    그 역할을 한다. 한 단어를 끊어서 넘기면 학습과 같은 조건이 되고,
    슬라이딩 윈도우를 넘기면 그 윈도우 안에서만 계산되어 영상 전체 대비
    값이 조금 달라진다. 정확도 문제가 생기면 이 차이를 먼저 의심할 것.
"""
